...
document
  .getElementById("temperatureConvertBtn")
...
function areaFn() {
  ...
  document.getElementById("areaResult").textContent = `Result: ${result.toFixed(
    2
  )} ${toUnit}`;
}
...
function weightFn() {
  ...
  document.getElementById(
    "weightResult"
  ).textContent = `Result: ${result.toFixed(2)} ${toUnit}`;
}
...
function timeFn() {
  ...
  let result =
    valInput * (conversionFactors[toUnit] / conversionFactors[fromUnit]);
  document.getElementById("timeResult").textContent = `Result: ${result.toFixed(
    2
  )} ${toUnit}`;
}
